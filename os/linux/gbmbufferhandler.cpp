...
bool GbmBufferHandler::CreateBuffer(uint32_t w, uint32_t h, int format,
  ...
  if (rbc_enabled) {
    mod = gbm_bo_get_plane_format_modifier(bo, plane);
    temp->meta_data_.format_modifiers_[2 * plane] =
        static_cast<uint32_t>(mod >> 32);
    temp->meta_data_.format_modifiers_[2 * plane + 1] =
  ...
  if (rbc_enabled) {
    ...
    for (size_t i = 0; i < temp->total_planes; i++) {
      ...
      temp->import_data.fd_modifier_data.strides[i] = gbm_bo_get_modifier(bo);
      temp->meta_data_.format_modifiers_[2 * plane] = modifier_low;
      temp->meta_data_.format_modifiers_[2 * plane + 1] = modifier_high;
    }
  } else {
...
#else
    if (!handle->meta_data_.format_modifiers[0]) {
...
void GbmBufferHandler::CopyHandle(HWCNativeHandle source,
  ...
  if (source->modifier) {
    temp->meta_data_.format_modifiers_[2 * plane] =
        source->meta_data_.format_modifiers_[2 * plane];
    temp->meta_data_.format_modifiers_[2 * plane + 1] =
        source->meta_data_.format_modifiers_[2 * plane + 1];
  }
#else
  if (!source->meta_data_.format_modifiers[0]) {
...
bool GbmBufferHandler::ImportBuffer(HWCNativeHandle handle) const {
  ...
  bool use_modifer = handle->meta_data_.format_modifiers_[0];
  uint64_t mod = 0;
  ...
  for (size_t i = 0; i < total_planes; i++) {
    ...
    mod = gbm_bo_get_plane_format_modifier(bo, plane);
    bo->meta_data_.format_modifiers_[2 * plane] =
        static_cast<uint32_t>(mod >> 32);
    bo->meta_data_.format_modifiers_[2 * plane + 1] =
        static_cast<uint32_t>(mod);
  }
#else
  if (!use_modifer) {
  ...
  uint32_t modifier_low =
      static_cast<uint32_t>(temp->import_data.fd_modifier_data.modifier >> 32);
  uint32_t modifier_high = static_cast<uint32_t>(mod);
  ...
  for (size_t i = 0; i < total_planes; i++) {
    ...
    bo->meta_data_.format_modifiers_[2 * plane] = modifier_low;
    bo->meta_data_.format_modifiers_[2 * plane + 1] = modifier_high;
  }
#endif
